  ...
  (:durative-action put_down_to_side_loader_dual
    ...
    (:durative-action switch_group
     ...
     :effect (and (at end (not (group_active ?ga)))
                  ...
                  (at start (increase (group-cost) 1))
     ))
    ...
    (:durative-action charge_mover
     :parameters (?m - mover ?st - charging_station)
     :duration (= ?duration (/ (- 20 (battery ?m)) (charging_vel ?st))) ;; no duration specified to charge
     ...
     :effect (and (at start (not (free ?st)))
                  ...
                  (at end (free ?st))
                  (at end (at_pause ?m))
                  (at end (increase (battery ?m) (- 20 (battery ?m)))))
     )

...
(:requirements :typing :durative-actions :numeric-fluents :negative-preconditions)
  ...
  (:durative-action pick_up_single
    :parameters (?r - mover ?c - normal_crate ?l - location)
  ...
  (:durative-action load_crate
    :parameters (?r - loader ?c - normal_crate ?l - location)
    :duration (= ?duration 4)
    :condition (and
      (at start (robot_at ?r ?l))
      (at start (crate_at ?c ?l))
  ...
  ;; Load Fragile Crate onto Conveyor Belt (Loader)
  (:durative-action load_crate_fragile
    :parameters (?r - loader ?c - fragile_crate ?l - location)
    :duration (= ?duration 6)
    :condition (and
      (at start (robot_at ?r ?l))
      (at start (crate_at ?c ?l))
  ...
  (:durative-action move_crate_single
    :parameters (?r - mover ?c - normal_crate ?from ?to - location)
